...
import { UploadCloud, X, Film, AlertCircle, RefreshCw } from 'lucide-react';
    ...
    <div className="w-full">
      <div className="p-6 md:p-8 relative">
        {!file ? (
          ...
          >
            ...
            <h3 className="text-xl font-bold text-slate-800 dark:text-slate-200 mb-2">Upload Video for Analysis</h3>
            <p className="text-slate-500 dark:text-slate-400 text-sm max-w-sm mb-6">
              Drag and drop a video file here, or click to browse. Supports MP4, WebM, MOV up to 10MB.
            </p>
        ...
        {loading && (
          <div className="absolute inset-0 bg-white/80 dark:bg-slate-900/60 backdrop-blur-sm rounded-2xl flex flex-col items-center justify-center border border-purple-500/30 overflow-hidden z-10">
            <div className="absolute top-0 left-0 w-full h-1 bg-purple-500 shadow-[0_0_15px_#a855f7] animate-[scan_2s_ease-in-out_infinite]" />
            <RefreshCw className="w-8 h-8 text-purple-500 dark:text-purple-400 animate-spin mb-3" />
            <span className="text-purple-600 dark:text-purple-300 font-medium tracking-wide animate-pulse">
              Analyzing video frames and facial artifacts...
            </span>
          </div>
        )}
      </div>
      ...
      <div className="bg-slate-50 dark:bg-slate-900/60 px-6 md:px-8 py-5 border-t border-slate-200 dark:border-slate-700/50 flex justify-end gap-4">
        ...
        <button
          ...
          disabled={loading || !file}
          className="flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white rounded-xl font-bold transition-all shadow-lg shadow-purple-500/25 disabled:opacity-50 disabled:cursor-not-allowed hover:-translate-y-0.5 active:translate-y-0"
        >
